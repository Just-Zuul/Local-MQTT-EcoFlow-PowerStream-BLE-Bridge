/* =========================================================================
   ...
   !! Required:  NimBLE-Arduino 2.x , PubSubClient !!
...
// =================== CONFIG  (fill in from here) ===================
//  --- shared by all PowerStreams ---
// WiFi credentials as a PRIORITY LIST (topmost entry = preferred).
// 1 entry = classic single network. 2+ entries = multiple SSIDs each with its own PW
// (e.g. dedicated box on top + mesh as fallback). Same IP network assumed (except when using DHCP).
...
// WiFi roaming protection :
//   0 = Auto   1 = Best-at-Boot (recommended)   2 = Hard-Lock (fixed BSSID)
#define WIFI_MODE  1
...
// Optional static IP (adjust per device!). false = DHCP.
static const bool USE_STATIC_IP = true;                // !!!! <----- !! NOTE !! - !! IMPORTANT !! ---- !!!!!
IPAddress local_IP(192, 168, 100, 100);                // !!!!! change per device
IPAddress gateway (192, 168, 100,   1);
IPAddress subnet  (255, 255, 255,   0);
IPAddress dns1    (192, 168, 100,   1);
IPAddress dns2    (192, 168, 100,   1);

//  --- MQTT Broker ---                                // fill in !! Caution with DHCP and fallback WiFi networks !! must be reachable !!
const char* MQTT_HOST = "192.168.100.200";             // Mosquitto host / HA-MQ  
const int   MQTT_PORT = 1883;
const char* MQTT_USER = "mqtt-User";                   // empty "" if none
const char* MQTT_PASS = "mqtt-PWD";                    // empty "" if none

//  --- EcoFlow User ID ---
const char* EF_USER_ID = "0000000000000000000";        // EcoFlow User ID (same for all devices)

// --- NTP / time ---
static const char* NTP_SERVER_1 = "192.168.100.1";               // router or other time source on the local LAN
static const char* NTP_SERVER_2 = "pool.ntp.org";                // fallback - public time source
static const char* TZ_STRING    = "CET-1CEST,M3.5.0,M10.5.0/3";  // timezone : CET/CEST incl. daylight saving

//  --- individual per device ---
const char* PS_SN     = "HW51ZOH4PS000000";    // serial number of the inverter
const char* PS_MAC    = "77:66:ef:44:zz:ee";   // BLE MAC of the inverter (lowercase!!)
const char* DEVICE_ID = "WR0-PS0000";          // short ID -> topics + MQTT client ID - composed as: PowerStream-WR0-PS0000

//  -----------------------------------------------------------------------------------------------
//  --- Below here, only change things if the impact is clear ---
//  --- behaviour ---
const int   START_WATTS = 0;                   // initial setpoint - inverter output at ESP start until MQTT says otherwise
const int   MIN_WATTS   = 0;                   // safety limit MIN
const int   MAX_WATTS   = 800;                 // limit on max watt output (mind the EF/PS specifications)
const uint32_t CONTROL_CHECK_MS = 5000;        // target/actual check interval (read/compare only)
const int      CONTROL_TOL_W    = 5;           // target/actual: tolerance (W) - also the write dead band
const uint32_t WRITE_GRACE_MS   = 6000;        // min gap between re-asserts (heartbeat latency, no thrash)
const uint32_t STATUS_MS        = 15000;       // link/RSSI status publish
const uint32_t TELEMETRY_MS     = 3000;        // publish permanent_watts/inv_output
const uint32_t BLE_RETRY_MS     = 8000;        // pause between BLE reconnect attempts

// --- WiFi emergency / failsafe (V2.6.2) ---
const int      FAILSAFE_WATTS   = 50;          // safe setpoint when WiFi is gone for longer (set via BLE)
const uint32_t FAILSAFE_WLAN_MS = 15000;       // WiFi gone CONTINUOUSLY this long -> failsafe setpoint
const uint32_t WIFI_RELOCK_MS   = 30000;       // WiFi gone this long (mode 1 only) -> re-evaluate AP (rescan/re-lock)

// Diagnostic expire_after (seconds!). Rule of thumb: >= 2x status interval, otherwise flicker.
// Auto-scales with STATUS_MS: 3 status cycles + 5 s reserve. (STATUS_MS is in ms -> /1000)
const uint32_t DIAG_EXPIRE_S    = 3 * STATUS_MS / 1000 + 5;   // = 50 s at STATUS_MS=15000
