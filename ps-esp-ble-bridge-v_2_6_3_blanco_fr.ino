/* =========================================================================
   ...
   -------------------------------------------------------------------------
   Modele vierge - (FR) Exemple : la configuration commence a la ligne 87.
   Remplace le decodeur TCP-MQTT par un pont BLE local :
   Lit toute la telemetrie du PowerStream et pilote consigne / luminosite / mode d'alimentation
   via BLE - independamment de la connexion Wi-Fi/cloud du PowerStream.
   ...
   !! Requis :  NimBLE-Arduino 2.x , PubSubClient !!
...
// =================== CONFIG  (a remplir a partir d'ici) ===================
//  --- commun a tous les PowerStream ---
// Identifiants WiFi en LISTE DE PRIORITE (premiere entree = preferee).
// 1 entree = reseau unique classique. 2+ entrees = plusieurs SSID, chacun avec son propre MDP
// (p. ex. box dediee en haut + mesh en secours). Meme reseau IP suppose (sauf en cas de DHCP).
struct WifiCred { const char* ssid; const char* pass; };
const WifiCred WIFI_CREDS[] = {
  { "SSID 1", "PASS 1" },                      // Prio 1 (preferee, p. ex. routeur ou AP dedie)
//  { "SSID 2", "PASS 2" },                    // Prio 2 (secours, p. ex. mesh) - si 1 seul reseau -> commenter la ligne
//  { "SSID 3", "PASS 3" },                    // Prio 3 (autre secours possible) - si 1 seul reseau -> commenter la ligne  
};
const int NUM_WIFI_CREDS = sizeof(WIFI_CREDS)/sizeof(WIFI_CREDS[0]);

// Protection contre le roaming WiFi :
//   0 = Auto   1 = Best-at-Boot (recommande)   2 = Hard-Lock (BSSID fixe)
#define WIFI_MODE  1
...
// IP statique optionnelle (a adapter par appareil !). false = DHCP.
static const bool USE_STATIC_IP = true;                // !!!! <----- !! NOTE !! - !! IMPORTANT !! ---- !!!!!
IPAddress local_IP(192, 168, 100, 100);                // !!!!! a changer par appareil
IPAddress gateway (192, 168, 100,   1);
IPAddress subnet  (255, 255, 255,   0);
IPAddress dns1    (192, 168, 100,   1);
IPAddress dns2    (192, 168, 100,   1);

//  --- MQTT Broker ---                                // a remplir !! Attention avec le DHCP et les reseaux WiFi de secours !! doit etre joignable !!
const char* MQTT_HOST = "192.168.100.200";             // hote Mosquitto / HA-MQ  
const int   MQTT_PORT = 1883;
const char* MQTT_USER = "mqtt-User";                   // vide "" si aucun
const char* MQTT_PASS = "mqtt-PWD";                    // vide "" si aucun

//  --- EcoFlow User ID ---
const char* EF_USER_ID = "0000000000000000000";        // EcoFlow User ID (identique pour tous les appareils)

// --- NTP / heure ---
static const char* NTP_SERVER_1 = "192.168.100.1";               // routeur ou autre source d'heure sur le LAN local
static const char* NTP_SERVER_2 = "pool.ntp.org";                // secours - source d'heure publique
static const char* TZ_STRING    = "CET-1CEST,M3.5.0,M10.5.0/3";  // fuseau horaire : CET/CEST avec heure d'ete

//  --- propre a chaque appareil ---
const char* PS_SN     = "HW51ZOH4PS000000";    // numero de serie de l'onduleur
const char* PS_MAC    = "77:66:ef:44:zz:ee";   // MAC BLE de l'onduleur (minuscules !!)
const char* DEVICE_ID = "WR0-PS0000";          // ID court -> topics + ID client MQTT - compose ainsi : PowerStream-WR0-PS0000

//  -----------------------------------------------------------------------------------------------
//  --- En dessous, ne modifier que si l'impact est clair ---
//  --- comportement ---
const int   START_WATTS = 0;                   // consigne initiale - sortie de l'onduleur au demarrage de l'ESP jusqu'a ce que MQTT impose autre chose
const int   MIN_WATTS   = 0;                   // limite de securite MIN
const int   MAX_WATTS   = 800;                 // limite de la puissance max en watts (respecter les specifications EF/PS)
const uint32_t CONTROL_CHECK_MS = 5000;        // intervalle de verification consigne/reel (lecture/comparaison uniquement)
const int      CONTROL_TOL_W    = 5;           // consigne/reel : tolerance (W) - sert aussi de bande morte d'ecriture
const uint32_t WRITE_GRACE_MS   = 6000;        // ecart min entre re-ecritures (latence heartbeat, pas de thrash)
const uint32_t STATUS_MS        = 15000;       // publication du statut link/RSSI
const uint32_t TELEMETRY_MS     = 3000;        // publier permanent_watts/inv_output
const uint32_t BLE_RETRY_MS     = 8000;        // pause entre les tentatives de reconnexion BLE

// --- urgence WiFi / failsafe (V2.6.2) ---
const int      FAILSAFE_WATTS   = 50;          // consigne de securite quand le WiFi est absent longtemps (reglee via BLE)
const uint32_t FAILSAFE_WLAN_MS = 15000;       // WiFi absent EN CONTINU pendant ce temps -> consigne failsafe
const uint32_t WIFI_RELOCK_MS   = 30000;       // WiFi absent pendant ce temps (mode 1 uniquement) -> reevaluer l'AP (rescan/re-lock)

// expire_after diagnostic (secondes !). Regle : >= 2x l'intervalle de statut, sinon scintillement.
// S'adapte auto a STATUS_MS : 3 cycles de statut + 5 s de reserve. (STATUS_MS en ms -> /1000)
const uint32_t DIAG_EXPIRE_S    = 3 * STATUS_MS / 1000 + 5;   // = 50 s a STATUS_MS=15000

// =================================================================
// --------------  ! Ab hier keine Änderungen !  -------------------
// --------  ! NE RIEN MODIFIER AU-DELA DE CE POINT  ! ---------
// =================================================================
...
static const Sensor SENSORS[] = {
  ...
  { "inv_volt_in",  "Tension d'entree onduleur",  35, 100, "V",  "voltage"     },
  { "inv_volt_op",  "Tension de service onduleur",  36, 10,  "V",  "voltage"     },
  { "inv_cur_out",  "Courant de sortie onduleur",     37, 1000,"A",  "current"     },
  { "inv",          "Sortie onduleur",           38, 10,  "W",  "power"       },  // V2.4-Key
  { "inv_temp",     "Temperature onduleur",        39, 10,  "°C", "temperature" },
  { "inv_freq",     "Frequence onduleur",          40, 10,  "Hz", "frequency"   },
  { "inv_dc_cur",   "Courant CC onduleur",          41, 1000,"A",  "current"     },
  // --- PV1 (input/10, op/100) ---
  { "pv1_volt_in",  "Tension d'entree PV1",       16, 10,  "V",  "voltage"     },
  { "pv1_volt_op",  "Tension de service PV1",       17, 100, "V",  "voltage"     },
  { "pv1_cur",      "Courant PV1",                  18, 10,  "A",  "current"     },
  { "pv1_watts",    "Puissance PV1",               19, 10,  "W",  "power"       },
  { "pv1_temp",     "Temperature PV1",             20, 10,  "°C", "temperature" },
  // --- PV2 (input/10, op/100) ---
  { "pv2_volt_in",  "Tension d'entree PV2",       21, 10,  "V",  "voltage"     },
  { "pv2_volt_op",  "Tension de service PV2",       22, 100, "V",  "voltage"     },
  { "pv2_cur",      "Courant PV2",                  23, 10,  "A",  "current"     },
  { "pv2_watts",    "Puissance PV2",               24, 10,  "W",  "power"       },
  { "pv2_temp",     "Temperature PV2",             25, 10,  "°C", "temperature" },
  // --- LLC (input/10, op/100) ---
  { "llc_volt_in",  "Tension d'entree LLC",       32, 10,  "V",  "voltage"     },
  { "llc_volt_op",  "Tension de service LLC",       33, 100, "V",  "voltage"     },
  { "llc_temp",     "Temperature LLC",             34, 10,  "°C", "temperature" },
  // --- Leistung / sonstiges ---
  { "perm",         "Watts permanents",            48, 10,  "W",  "power"       },  // V2.4-Key
  { "rated_power",  "Puissance nominale",               58, 10,  "W",  "power"       },
  { "bat_input_watts","Puissance batterie",        29, 10,  "W",  "power"       },  // Feld 29, vorzeichenbehaftet: +/- = Fluss zur/von Batterie (wie Decoder)
  { "hb_freq",      "Frequence heartbeat",         57, 1,   "s",  "duration"    },
  // --- Status / Roh-Codes (alle sichtbar) ---
  { "inv_onoff",    "Onduleur marche/arret",            53, 1,   "",   ""            },
  { "inv_status",   "Etat onduleur",            15, 1,   "",   ""            },
  { "inv_relay",    "Etat relais onduleur",      43, 1,   "",   ""            },
  { "bp_type",      "Type de pack batterie",          42, 1,   "",   ""            },  // Inverter meldet Pack-Typ: 2 = DP verbunden/identifiziert
  { "pv1_status",   "Etat PV1",                 11, 1,   "",   ""            },
  { "pv1_relay",    "Etat relais PV1",           44, 1,   "",   ""            },
  { "pv2_status",   "Etat PV2",                 12, 1,   "",   ""            },
  { "pv2_relay",    "Etat relais PV2",           45, 1,   "",   ""            },
  { "llc_status",   "Etat LLC",                 14, 1,   "",   ""            },
  { "supply_prio",  "Priorite d'alimentation",            50, 1,   "",   ""            },
  { "install_ctry", "Pays d'installation",       46, 1,   "",   ""            },
  { "install_town", "Ville d'installation",          47, 1,   "",   ""            },
  // --- Error/Warning Codes ---
  { "inv_err",      "Code d'erreur onduleur",        1,  1,   "",   ""            },
  { "inv_warn",     "Code d'avertissement onduleur",      3,  1,   "",   ""            },
  { "pv1_err",      "Code d'erreur PV1",             2,  1,   "",   ""            },
  { "pv1_warn",     "Code d'avertissement PV1",           4,  1,   "",   ""            },
  { "pv2_err",      "Code d'erreur PV2",             5,  1,   "",   ""            },
  { "pv2_warn",     "Code d'avertissement PV2",           6,  1,   "",   ""            },
  { "llc_err",      "Code d'erreur LLC",             9,  1,   "",   ""            },
  { "llc_warn",     "Code d'avertissement LLC",           10, 1,   "",   ""            },
  { "wl_err",       "Code d'erreur sans fil",        54, 1,   "",   ""            },
  { "wl_warn",      "Code d'avertissement sans fil",      55, 1,   "",   ""            },
};
...
// ----------------- Auto-test crypto -----------------
bool selfTest(){
  Serial.println(F("--- Auto-test crypto ---"));
  bool ok=true;
  ...
  Serial.println(ok?F("--- Auto-test REUSSI ---"):F("--- Auto-test ECHOUE ---"));
  return ok;
}
...
class ClientCB:public NimBLEClientCallbacks{
  void onConnect(NimBLEClient* c)override{ wsLog("[BLE] connecte"); }
  void onDisconnect(NimBLEClient* c,int reason)override{
    wsLog("[BLE] deconnecte raison=0x%X", reason);
...
void bleSetWatts(int w){
  ...
  if(sendFrame(f,n)) wsLog("[BLE] set_load_power(%dW) envoye",w);
}
...
void regulate(){
  ...
  wsLog("[Ctrl] consigne=%dW reel=%dW %s -> ecriture", soll, g_permWatts, changed?"(nouv.)":"(ecart)");
  bleSetWatts(soll);
...
void bleSetBright(int pct){
  ...
  if(sendFrame(f,n)) wsLog("[BLE] set_brightness(%d%% -> %d bits) envoye",pct,bits);
}
void bleSetSupply(int val){
  uint8_t f[64]; size_t n=fSetSupply(f,val);
  if(sendFrame(f,n)) wsLog("[BLE] set_supply_priority(%d) envoye",val);
}
bool bleConnectAndAuth(){
  wsLog("[BLE] connexion a %s ...", PS_MAC);
  if(!g_client) { g_client=NimBLEDevice::createClient(); g_client->setClientCallbacks(new ClientCB(),false); }
  if(!g_client->connect(NimBLEAddress(PS_MAC,BLE_ADDR_PUBLIC))){ wsLog("[BLE] echec connexion"); return false; }
  NimBLERemoteService* svc=g_client->getService(SVC_UUID);
  if(!svc){ wsLog("[BLE] aucun service"); g_client->disconnect(); return false; }
  g_write=svc->getCharacteristic(WRITE_UUID); g_notify=svc->getCharacteristic(NOTIFY_UUID);
  if(!g_write||!g_notify){ wsLog("[BLE] aucune caracteristique"); g_client->disconnect(); return false; }
  if(!g_notify->subscribe(true,notifyCB)){ wsLog("[BLE] echec subscribe"); g_client->disconnect(); return false; }
  ...
  if(!g_authOk){ wsLog("[BLE] auth non confirmee"); g_client->disconnect(); return false; }
  g_bleReconnects++;
...
int pickBestCred(uint8_t* outB, int* outRssi){
  wsLog("[WiFi] scan de %d reseau(x) configure(s)...", NUM_WIFI_CREDS);
  WiFi.mode(WIFI_STA); WiFi.disconnect(); delay(100);
  int n=WiFi.scanNetworks(false,false);
  if(n<=0){ wsLog("[WiFi] aucun reseau"); return -1; }
  int found=-1;
  for(int c=0; c<NUM_WIFI_CREDS && found<0; c++){
    ...
    if(idx>=0){
      ...
      wsLog("[WiFi] '%s' (prio %d) meilleur AP %s @ %d dBm", WIFI_CREDS[c].ssid, c+1, m, best);
    }
  }
  if(found<0) wsLog("[WiFi] aucun des SSID configures trouve");
...
void wifiModeInit(){
  ...
  if(WIFI_MODE==1){
    ...
    else wsLog("[WiFi] echec scan mode1 -> Auto (prio 1)");
  } else if(WIFI_MODE==2){
    g_credIdx=0; memcpy(g_lockedBssid,AP_BSSID,6); g_bssidLockActive=true;
    char m[18]; formatBssid(AP_BSSID,m,sizeof(m));
    wsLog("[WiFi] mode2 hard-lock sur %s (%s, identifiants prio-1)", m, AP_NAME);
  } else {
    uint8_t b[6]; int r; int c=pickBestCred(b,&r);   // Auto: Netz waehlen (Box-Prio), aber kein BSSID-Lock
    if(c>=0) g_credIdx=c;
    wsLog("[WiFi] mode0 Auto (reseau : %s)", WIFI_CREDS[g_credIdx].ssid);
  }
}
void wifiEnsure(){
  if(WiFi.status()==WL_CONNECTED){
    g_wlanDownSince=0;
    if(g_failsafeActive){ g_failsafeActive=false; wsLog("[Failsafe] WiFi de retour -> HA reprend la main"); }
    if(g_wifiConnecting){
    ...
    if(!g_ntpStarted){
      ...
      g_ntpStarted=true;
      wsLog("[NTP] sync demandee (%s, TZ %s)", NTP_SERVER_1, TZ_STRING);
    }
    return;
  }
  ...
  if(!g_failsafeActive && down>=FAILSAFE_WLAN_MS){
    ...
    wsLog("[Failsafe] WiFi absent >%us -> consigne %dW", (unsigned)(FAILSAFE_WLAN_MS/1000), FAILSAFE_WATTS);
  ...
  if(WIFI_MODE!=2 && down>=WIFI_RELOCK_MS){
    wsLog("[WiFi] WiFi absent >%us -> reevaluer reseau/AP (rescan)", (unsigned)(WIFI_RELOCK_MS/1000));
  ...
  if(g_wifiConnecting){
    if(millis()-g_wifiConnectStart>20000){ wsLog("[WiFi] delai de connexion depasse"); WiFi.disconnect(); g_wifiConnecting=false; }
    return;
  }
  ...
  if(g_bssidLockActive){
    ...
    wsLog("[WiFi] connexion '%s' (Lock %s)...", cr.ssid, m);
  } else { WiFi.begin(cr.ssid,cr.pass); wsLog("[WiFi] connexion '%s' (Auto)...", cr.ssid); }
...
void publishTelemetry(){
  ...
  for(int i=0;i<NUM_SENSORS;i++){
    ...
    if(strcmp(s.key,"inv_onoff")==0){                          // 1/0 -> Text (Wunsch)
      g_mqtt.publish(t.c_str(), raw?"marche":"arret", true);
      continue;
    }
    char val[24];
  ...
  { int sp = g_fieldSeen[50] ? (int)g_field[50] : 0;
    g_mqtt.publish(TOPIC_SUPPLYSTATE.c_str(),
                   sp==0 ? "Prioriser l'alimentation" : "Prioriser le stockage", true); }
  publishBrightState();
}
...
static const Diag DIAGS[] = {
  { "brssi",     "RSSI BLE",                 "dBm", "signal_strength" },
  { "heap",      "Memoire libre",              "B",   ""                },
  { "breconn",   "Reconnexions BLE",           "",    ""                },
  { "mreconn",   "Reconnexions MQTT",          "",    ""                },
  { "uptime",    "Uptime",                   "s",   "duration"        },
  { "bdisc",     "Raison deconnexion BLE",     "",    ""                },
  { "ip",        "Adresse IP WiFi",          "",    ""                },
  { "ssid",      "SSID WiFi",                "",    ""                },
  { "wmac",      "MAC WiFi (ESP)",           "",    ""                },
  { "bssid",     "BSSID AP (actuel)",       "",    ""                },
  { "bssid_cfg", "BSSID AP (configure)",  "",    ""                },
  { "wmode",     "Mode WiFi",               "",    ""                },
  { "apname",    "Nom AP",                  "",    ""                },
  { "block",     "Verrou BSSID",               "",    ""                },
  { "bmac_esp",  "MAC BLE (ESP)",            "",    ""                },
  { "bmac_inv",  "MAC BLE (onduleur)",       "",    ""                },
  { "reset",     "Raison du redemarrage",           "",    ""                },
  { "fw",        "Firmware",                 "",    ""                },
};
...
void publishFixedDiscovery(){
  ...
  p  = "{\"name\":\"Consigne d'injection\",\"uniq_id\":\"ps_"; p+=DEVICE_ID; p+="_set\",";
  ...
  p  = "{\"name\":\"Luminosite onduleur\",\"uniq_id\":\"ps_"; p+=DEVICE_ID; p+="_bright\",";
  ...
  p += "\"options\":[\"Prioriser l'alimentation\",\"Prioriser le stockage\"],";
  p += "\"icon\":\"mdi:transmission-tower-import\","+dev+"}";
  g_mqtt.publish(t,p.c_str(),true);

  snprintf(t,sizeof(t),"homeassistant/button/ps_%s/restart/config",DEVICE_ID);
  p  = "{\"name\":\"Redemarrage ESP\",\"uniq_id\":\"ps_"; p+=DEVICE_ID; p+="_restart\",";
  ...
  p  = "{\"name\":\"RSSI WiFi\",\"uniq_id\":\"ps_"; p+=DEVICE_ID; p+="_rssi\",";
...
void stepDiscovery(){
  ...
  for(int b=0; b<BATCH && g_discStep>=0; b++){
    ...
    wsLog("[MQTT] discovery termine (%d capteurs + %d diagnostics + controles)", NUM_SENSORS, NUM_DIAG);
  }
}
...
void mqttCallback(char* topic,byte* payload,unsigned int len){
  ...
  if(strcmp(topic,TOPIC_SET.c_str())==0){
    ...
    wsLog("[MQTT] consigne recue : %dW", w);
  ...
  if(strcmp(topic,TOPIC_BRIGHT.c_str())==0){
    ...
    wsLog("[MQTT] luminosite recue : %d%%", pct);
    g_targetBright=pct;
  ...
  if(strcmp(topic,TOPIC_SUPPLY.c_str())==0){
    ...
    int val = (strcmp(v,"Prioriser le stockage")==0) ? 1 : 0;   // sonst supply(0)
    wsLog("[MQTT] mode d'alim. recu : %s -> %d", v, val);
    g_targetSupply=val;
  ...
  if(strcmp(topic,TOPIC_RESTART.c_str())==0){
    wsLog("[CMD] redemarrage ESP demande -> reboot");
...
void mqttEnsure(){
  ...
  } else wsLog("[MQTT] echec connexion rc=%d", g_mqtt.state());
}
...
void handleRoot(){
  ...
  h += " &middot; "; h += String(NUM_SENSORS); h += " capteurs &middot; heure "; h += nowClock(); h += "</div>";

  h += "<div class='grid'>";
  h += statCard("Consigne", String(g_targetWatts)+" W");
  h += statCard("Permanent (reel)", g_permWatts>=0 ? (String(g_permWatts)+" W") : String("&ndash;"));
  h += statCard("Sortie onduleur", g_invOutput>=0 ? (String(g_invOutput)+" W") : String("&ndash;"));
  h += statCard("Luminosite", g_curBrightPct>=0 ? (String(g_curBrightPct)+" %") : String("&ndash;"));
  const char* bleTxt; const char* bleCls;
  if (g_bleConnected && g_authOk)      { bleTxt = "connecte &amp; auth"; bleCls = "ok"; }
  else if (g_bleConnected)             { bleTxt = "connecte, sans auth";    bleCls = "warn"; }
  else                                 { bleTxt = "deconnecte";              bleCls = "err"; }
  h += statCard("BLE", bleTxt, bleCls);
  if (WiFi.status() == WL_CONNECTED) h += statCard("WiFi", String(WiFi.RSSI())+" dBm", "ok");
  else                               h += statCard("WiFi", "deconnecte", "err");
  h += statCard("MQTT", g_mqtt.connected() ? "connecte" : "deconnecte", g_mqtt.connected() ? "ok" : "err");
...
void setup(){
  ...
  if(!selfTest()){Serial.println(F("STOP : auto-test crypto echoue."));return;}
  wsLog("Boot : PowerStream Bridge V%s (Appareil %s)", FW_VERSION, DEVICE_ID);
  ...
  wsLog("Topic de base : %s", base.c_str());
  ...
  else                               wsLog("[Web] demarre, en attente du WiFi");
